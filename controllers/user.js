...
const register = async (req, res, next) => {
  ...
  await user.save();
  //3. Sending email
  const token = await signToken({ _id: user._id, fromEmail: true });
  ...
  return res.status(200).json({
    success: true,
    message: "Register success! Please check email to activate user",
    data: null,
  });
};
...
const login = async (req, res, next) => {
  const user = req.user;
  //Sign token
  const token = await signToken({ _id: user._id, loginAt: user.loginAt });
  //Output
  return res.status(200).json({
    success: true,
